    ...
    return (
      <Spin spinning={this.state.isLoading}>
        <Form
          ...
          onFinish={this.save}
          onValuesChange={(changedValues, allValues) => {
            const { orderItems } = allValues;
            if (!orderItems || orderItems.length === 0) return;
          
            const updatedorderItems = [...orderItems];
          
            // Safely calculate totalAmount
            const totalAmount = updatedorderItems
              .filter((item) => item != null) // Ensure valid items
              .reduce(
                (sum, item) =>
            ...
            // Safely calculate balanceAmount
            const balanceAmount =
              (Number(totalAmount) || 0) - (Number(allValues.advanceAmount) || 0);
